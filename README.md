...
- Frontend: 静的SPA
- Backend: Cloudflare Workers または Node.js on Raspberry Pi
- Auth: Google Identity Services + Worker側IDトークン検証
- Data: Google Sheets
- Calendar: RED BISONS共有Googleカレンダー
- Secrets: Cloudflare Worker secrets または Raspberry Pi上の `.env`
...
自宅のRaspberry Piを一時的な本番サーバーにできます。Node.jsサーバーとDocker構成を用意しています。
...
初期運用ではRaspberry Piで検証し、運用が固まったらCloudflare WorkersやVPSなどへ環境変数とDNSを移して委譲できます。
