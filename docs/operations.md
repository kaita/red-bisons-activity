...
画面全体を見たい場合は、ローカル限定のデモモードを使う。

```bash
npm run dev:demo
```

ブラウザで `http://localhost:8787` を開き、「デモで見る」を押す。サンプル活動、参加者一覧、見守り不足表示、回答フォームを確認できる。

`DEMO_MODE=true` は本番環境では使わない。
...
- Cloudflare Budget alertを作成済み
- Google Cloud Budget alertを作成済み
...
課金監視の詳細は [billing-monitoring.md](billing-monitoring.md) を参照する。
